...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Handle image upload
    $picture = $data["Picture"] ?? '';  // Default picture if $data isn't available
...
// Check if the query returns a valid row
if ($result && mysqli_num_rows($result) > 0) {
    // Initialize $data with the query result
    $data = mysqli_fetch_array($result);
} else {
    // If no data is found, handle the error
    echo "<p align='center'>ไม่พบข้อมูลหนังสือที่ต้องการแก้ไข</p>";
